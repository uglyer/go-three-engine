...
func NewBox3(min, max *Vector3) *Box3 {
	...
	b := &Box3{Min: NewVec3(), Max: NewVec3()}
	b.Set(min, max)
	return b
}

// NewBox3Infinity 创建无穷大的包围盒
func NewBox3Infinity() *Box3 {
	b := &Box3{Min: NewVec3(), Max: NewVec3()}
	b.Set(nil, nil)
	return b
}
...
func (b *Box3) Copy(other *Box3) *Box3 {
	...
	b.Min.Copy(other.Min)
	b.Max.Copy(other.Max)
	return b
}
...
// Returns pointer to this updated bounding box.
func (b *Box3) ExpandByPointXYZ(x, y, z float32) *Box3 {
	if b.Min.X > x {
		b.Min.X = x
	}
	if b.Min.Y > y {
...
func (b *Box3) Clone() *Box3 {
	...
	return NewBox3Infinity().Copy(b)
}

// HasInfinity 校验是否存在正无穷或负无穷的数字
func (b *Box3) HasInfinity() bool {
	return b.Min.X == -Infinity || b.Min.Y == -Infinity || b.Min.Z == -Infinity || b.Max.X == Infinity || b.Max.Y == Infinity || b.Max.Z == Infinity
}
