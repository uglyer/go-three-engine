...
func NewFrustum(p0, p1, p2, p3, p4, p5 *Plane) *Frustum {
	...
	f.planes = make([]*Plane, 6)
	for i := 0; i < 6; i++ {
		f.planes[i] = NewPlane(nil, 0)
	}
	f.Set(p0, p1, p2, p3, p4, p5)
	return f
}

// NewEmptyFrustum returns a pointer to a new Frustum object
func NewEmptyFrustum() *Frustum {
...
func (f *Frustum) Copy(frustum *Frustum) *Frustum {
	...
	for i := 0; i < 6; i++ {
		f.planes[i].Copy(frustum.planes[i])
	}
	return f
}
