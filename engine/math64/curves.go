...
func (c *Curve) GetLength() float64 {
	return c.length
}
...
func NewBezierQuadratic(origin, control, destination *Vector3, npoints int) *Curve {
	...
	var equation = func(t, v0, v1, v2 float64) float64 {
		a0 := 1.0 - t
	...
	for i := 0; i <= npoints; i++ {
		...
		bezier = append(bezier, NewVector3(x, y, z))
	}
...
func NewBezierCubic(origin, control1, control2, destination *Vector3, npoints int) *Curve {
	...
	var equation = func(t, v0, v1, v2, v3 float64) float64 {
		a0 := 1.0 - t
...
func NewHermiteSpline(origin, tangent1, destination, tangent2 *Vector3, npoints int) *Curve {
	...
	var equation = func(t float64, v0, tan0, v1, tan1 *Vector3) *Vector3 {
		t2 := t * t
		t3 := t * t2
	...
	var hermite []*Vector3
	for i := 0; i <= npoints; i++ {
		vect := equation(float64(i)*step, origin, tangent1, destination, tangent2)
...
func NewCatmullRomSpline(points []*Vector3, npoints int, closed bool) *Curve {
	...
	var equation = func(t float64, v0, v1, v2, v3 *Vector3) *Vector3 {
		t2 := t * t
		t3 := t * t2
